...
import gliderflight
import numpy as np
from latlon import convertToDecimal, convertToNmea
...
class GliderException(Exception):
    pass
...
class SlocumShallow100Hardware(object):
    def __init__(self):
        self.buoyancypump = LinearActuator(
            initial_position=200.0,
            speed=32.0,  # from data
            position_max=233.0,
            position_min=-233.0,
            deadbandWidth=10.0,
        )
        self.pitchmotor = LinearActuator(
        ...
        self.finmotor = LinearActuator(0.0, 0.02, 0.45, -0.45, 0.035)
...
class BaseGliderModel(object):
    ...
    def initialise_gliderflightmodel(
        self, Cd0, Vg, mg, T1=1235, T2=-28.8, T3=0.14, T4=0, **kwds
    ):
    ...
    def initialise_gliderstate(self, datestr, timestr, lat, lon, mission_start):
        ...
        gs = {}
        gs["m_depth"] = 0
        gs["m_present_secs_into_mission"] = 0.0
        gs["samedepth_for"] = 0
        if datestr:
            if timestr:
                gs["m_present_time"] = arrow.get(
                    " ".join([datestr, timestr]), "YYYYMMDD HH:mm"
                ).timestamp()
            else:
                gs["m_present_time"] = arrow.get(datestr, "YYYYMMDD").timestamp()
        else:
            gs["m_present_time"] = 0.0
        gs["stack"] = 1  # number of commands given.
        gs["bpump_stack"] = 0
        gs["fin_stack"] = 0
        gs["pitch_stack"] = 0
        gs["m_ballast_pumped"] = 0.0
        gs["m_dist_to_wpt"] = 0.0
        ...
        gs["x_time_dive"] = 0
        gs["_init_dive_time"] = 80.0  # time for glider to open files/init devices. etc.
        gs["x_lat"] = 0
        gs["x_lon"] = 0
        gs["x_water_depth"] = 0
        gs["x_last_wpt_lat"] = 69696969.0
        gs["x_last_wpt_lon"] = 69696969.0
        gs["x_eastward_glider_velocity"] = 0.0
        gs["x_northward_glider_velocity"] = 0.0
        gs["x_upward_glider_velocity"] = 0.0
        ...
        gs["m_pressure"] = 0
        gs["x_lmc_x_wpt_calc"] = (
            0  # stores start point of transect, used to calculate heading from waypoints.
        )
        gs["x_lmc_y_wpt_calc"] = 0
        gs["_is_grounded"] = (
            0  # checked when needed to stop simulation if glider ran aground. Set in update.
        )
        self.gs = gs
    ...
    def update(self, t, dt):
        ...
        u_water, v_water, w_water, water_depth, eta, S, T, rho = (
            self.get_environmental_data(t, realLat, realLon, self.z)
        )
        ...
        p = [
            self.gs[i]
            for i in "m_present_time m_heading m_heading_rate m_fin m_speed".split()
        ]
        ...
        tmp = self.gliderflight_model.step_integrate(
            self.x,
            self.y,
            self.z,
        ...
        speed = (
            self.gs["x_eastward_glider_velocity"] ** 2
            + self.gs["x_northward_glider_velocity"] ** 2
        ) ** 0.5
        ...
        #
        self.gs["m_lat"], self.gs["m_lon"] = self.translate_lmc_latlon(
            self.lmc_x, self.lmc_y
        )
...
class FinModel(object):
    def __init__(self, c0=0.5, c1=15.0):
        self.c0 = c0
        self.c1 = c1
    ...
    def compute_heading_rate(self, dt, m_fin, m_speed, m_heading_rate):
        ...
        m_heading_rate = self.c0 / self.c1 * np.sin(
            m_fin
        ) * m_speed**2 * dt + m_heading_rate * (1 - dt / 2 / self.c1)
...
class GliderFlightModel(gliderflight.DynamicGliderModel):
    ...
    def __init__(
        ...
        super().__init__(
            dt, rho0, k1, k2, max_depth_considered_surface=max_depth_considered_surface
        )
        self.pitch_model_parameters = (
            1235,
            -28.8,
            0.138,
            0,
        )  # T1, T2, T3 and T4from glidertrim program.
    ...
    def step_integrate(self, x, y, z, u, v, w, pitch, rho, buoyancy_change, heading):
        ...
        Cd0 = self.Cd0
        # stage 1
        k1_u, k1_w = self._DynamicGliderModel__compute_k(
            uh, w, rho, pitch, FBg, m11, m12, m21, m22, h, Cd0
        )
        k1_sx = h * uh
        k1_sz = h * w
        _uh = uh + k1_u * 0.5
        _w = w + k1_w * 0.5
        # stage 2
        k2_u, k2_w = self._DynamicGliderModel__compute_k(
            _uh, _w, rho, pitch, FBg, m11, m12, m21, m22, h, Cd0
        )
        k2_sx = h * _uh
        k2_sz = h * _w
        # stage 3
        _uh = uh + k2_u * 0.5
        _w = w + k2_w * 0.5
        k3_u, k3_w = self._DynamicGliderModel__compute_k(
            _uh, _w, rho, pitch, FBg, m11, m12, m21, m22, h, Cd0
        )
        k3_sx = h * _uh
        k3_sz = h * _w
        ...
        _uh = uh + k3_u
        _w = w + k3_w
        k4_u, k4_w = self._DynamicGliderModel__compute_k(
            _uh, _w, rho, pitch, FBg, m11, m12, m21, m22, h, Cd0
        )
        k4_sx = h * _uh
        k4_sz = h * _w
    ...
    def compute_heading_from_fin(self, t, heading, heading_rate, m_fin, m_speed):
        heading_rate = self.fin_model.compute_heading_rate(
            self.dt, m_fin, m_speed, heading_rate
        )
        heading += heading_rate * self.dt
        heading %= np.pi * 2
        return heading, heading_rate

    def compute_pitch_from_battpos_buoyancy_drive(
        self, m_battpos, m_ballast_pumped, m_pressure
    ):
...
class Shallow200mExtendedGliderModel(BaseGliderModel, SlocumShallow200ExtendedHardware):
    def __init__(self):
