...
class GliderData(object):
    ...
    BATHYMETRY_FILENAME = (
        "/home/thopri/glidersim/gebco_2023_n68.291_s39.5508_w-31.8604_e32.5635.nc"
    )
    ...
    def __init__(
        self,
        glider_name,
        gliders_directory=None,
        bathymetry_filename=None,
    ...
    def read_bathymetry(self):
        ...
        bathymetry = (
            self.NC_ELEVATION_FACTOR
            * dataset.variables[self.NC_ELEVATION_NAME][...].copy()
        )
    ...
    def read_gliderdata(self, tm, lat, lon):
        path = os.path.join(
            self.gliders_directory,
            self.glider_name,
            "from-glider",
            "%s*.[st]bd" % (self.glider_name),
        )
        ...
        if self.glider_is_simulator:
            ...
            tmp = dbd.get_sync(
                *"sci_water_cond sci_water_temp sci_water_pressure".split()
            )
            age = tm - tmp[0]
            age_condition = np.logical_and(age >= 0, age < self.AGE * 3600)
            t, C, T, P = np.compress(
                np.logical_and(tmp[1] > 0, age_condition), tmp, axis=1
            )
            try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to get surface current estimate. Surface currents set to 0."
                )
                us = 0
                vs = 0
            u, v = np.compress(
                np.logical_and(np.abs(u) < 1.5, np.abs(v) < 1.5), [u, v], axis=1
            )
        dbd.close()
        ...
        self.rho_fun = interp1d(
            zj, rho_avg, bounds_error=False, fill_value=(rho_avg[0], rho_avg[-1])
        )
        self.SA_fun = interp1d(
            zj, SA_avg, bounds_error=False, fill_value=(SA_avg[0], SA_avg[-1])
        )
        self.T_fun = interp1d(
            zj, T_avg, bounds_error=False, fill_value=(T_avg[0], T_avg[-1])
        )

        if (
            self.u_fun is None
        ):  # not intialised yet, use last water current estimate available.
            if self.ekman_depth is None:
                logger.info(
                    "Setting water velocities with constant value equal to water_v{x,y}"
                )
                ...
                logger.info(
                    f"   including ekman depth of {self.ekman_depth} with us={us} and vs ={vs}."
                )
...
class DriftModel(GliderData):
    ...
    def download_drift_data(self, t, lat, lon):
        ...
        tstr = "{:4d}_{:02d}_{:02d}_{:02d}_{:02d}".format(
...
class VelocityRealityModel(GliderData):
    def __init__(
        self,
        glider_name: str,
        download_time: int = 12,
        gliders_directory: str = None,
        bathymetry_filename: str = None,
        spec_file: str = None,
    ):
        super().__init__(glider_name, gliders_directory, bathymetry_filename)
    ...
    def initialise_velocity_data(self, t: float, lat: float, lon: float):
        self.stores = get_extent(spec_file=self.spec_file)
        self.interpolators = create_interpolator(stores=self.stores)
    ...
    def get_data(self, t, lat, lon, z):
        ...
        point = {'latitude':lat, 'longitude':lon, "depth":-z, "time":dt_str}
        V = interpolate(interpolators=self.interpolators, point=point)
        # TODO default zero velocity is ok but these values won't always be sensibe
        # for salinity and temperature
        C = float(V["absolute_salinity"])
        if np.isnan(C):
            C = 34.50
        T = float(V["conservative_temperature"])
        if np.isnan(T):
            T = 15.00
        # Calculate pressure from depth (negative depth since it's below sea level)
        pressure = gsw.p_from_z(
            z, 0
        )  # Z is negative for underwater depth, 0 is for sea level
        ...
        abs_salinity = C #gsw.SA_from_SP(C, pressure, lon, lat)  # Salinity in PSU
        # calculate conservative temp from potential temperature and abs salinity
        cons_temp = T #gsw.CT_from_pt(abs_salinity, T)
        ...
        u = float(V["u_velocity"])
        if np.isnan(u):
            u = 0.00
        v = float(V["v_velocity"])
        if np.isnan(v):
            v = 0.00
        w = float(V["w_velocity"])
        if np.isnan(w):
            w = 0.00
        eta = 0
        S = abs_salinity
