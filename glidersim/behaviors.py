...
class Condition(object):
    ...
    def check_param(self, gs, parameter, value, valuerange, operator):
        ...
        if (value <= valuerange[1] and value >= valuerange[0]) or value == None:
    ...
    def check_params(self, gs):
        rs = []
        for p, v, vr, o in zip(
            self.parameter, self.value, self.valuerange, self.operator
        ):
...
class WhenBehavior(Behavior):
    def __init__(self, start_when, when_secs, when_wpt_dist):
    ...
    def init(self):
        ...
        if self.start_when == 0:  # immediately
            ...
            self.b_arg["start_when"] = Condition(
                "time_since_cycle_end", ">", self.when_secs, (0, None)
            )
        elif self.start_when == 7:  # close to waypoint
            self.b_arg["start_when"] = Condition(
                "m_dist_to_wpt", "<", self.when_wpt_dist, (0, None)
            )
        elif self.start_when == 9:  # Time since cycle start
            self.b_arg["start_when"] = Condition(
                "time_since_cycle_start", ">", self.when_secs, (0, None)
            )
        elif self.start_when == 12:  # No comms
            self.b_arg["start_when"] = Condition(
                "nocomms", ">", self.when_secs, (0, None)
            )
        elif self.start_when == 13:  # at UTC
            self.b_arg["start_when"] = UTC_Condition(
                (
                    self.when_utc_min,
                    self.when_utc_hour,
                    self.when_utc_day,
...
class DiveClimbBehavior(WhenBehavior):
    ...
    def init(self):
        ...
        self.diveclimb = UpDownSettings(
            self.use_bpump, self.bpump_value, self.use_pitch, self.pitch_value
        )
        # condition when to stop
        self.b_arg["stop_when"] = Condition(
            "m_altitude",
            "<",
            ...
            "hover_for",
            ">",
            self.stop_when_hover_for,
            (0, None),
            "stalled_for",
            ">",
            self.stop_when_stalled_for,
            (0, None),
        )
    ...
    def Complete(self, fsm):
        ...
        fsm.memory = []
        # reset any timers
        self.gliderState["hover_for"] = 0.0
        self.gliderState["stalled_for"] = 0
        self.gliderState["samedepth_for"] = 0.0
...
class UpDownSettings(object):
    def __init__(self, use_bpump, bpump_value, use_pitch, pitch_value):
        self.use_bpump = use_bpump
        self.bpump_value = bpump_value
        self.use_pitch = use_pitch
        self.pitch_value = pitch_value
    ...
    def get_pitch(self):
        if self.use_pitch == 3:
            ...
            raise ValueError(
                "Only use_pitch==3 implemented. Sorry. Perhaps disable error message?"
            )
...
class Abend(Behavior):
    def __init__(self, overdepth=-1, overtime=-1, samedepth_for=-1):
    ...
    def init(self):
        ...
        self.b_arg["overtime"] = Condition(
            "m_present_secs_into_mission", ">", self.overtime, (0, None)
        )
        # self.b_arg['samedepth_for']=ConditionAnd('samedepth_for','>',self.samedepth_for,(0,None),
        #                                         'm_depth','>',0.1,(None,None))
        self.b_arg["samedepth_for"] = Condition(
            "samedepth_for", ">", self.samedepth_for, (0, None)
        )
...
class Surface(WhenBehavior):
    ...
    def init(self):
        # WhenBehavior.__init__(self,self.start_when,self.when_secs,self.when_wpt_dist)
        WhenBehavior.init(self)
        self.climb = UpDownSettings(
            self.c_use_bpump, self.c_bpump_value, self.c_use_pitch, self.c_pitch_value
        )
        ...
        self.fsm.add_transition(
            "resume", "WaitForGPS", action=None, next_state="WaitForUser"
        )
        self.fsm.add_transition(
            "resume", "WaitForUser", self.WaitForFinalGPS, "WaitForFinalGPS"
        )
    ...
    def Complete(self, fsm):
        ...
        self.DRsurfacingLmc = (
            self.gliderState["m_lmc_x"],
            self.gliderState["m_lmc_y"],
        )  # dead reckoned
    ...
    def WaitForGPS(self, fsm):
        ...
        # reset hovering variables too
        self.gliderState["hover_for"] = 0.0
        self.gliderState["stalled_for"] = 0.0
        self.gliderState["samedepth_for"] = 0.0
        # leave this state if:
        if (
            self.gliderState["m_gps_status"] == 0
            or currentTime - self.surfacingTime > self.gps_wait_time
        ):
            self.b_arg["resume"].set_True()
            if self.gliderState["m_gps_status"] == 0:  # good fix:
                ...
                if dt == 0:
                    self.gliderState["m_water_vx"] = -1.0
                    self.gliderState["m_water_vy"] = -1.0
                else:
    ...
    def WaitForUser(self, fsm):
        ...
        if self.end_action == 2:  # resume
            ...
            if (
                surfaceTime > self.datatransfertime
                and surfaceTime > self.keystroke_wait_time
            ):
                self.b_arg["resume"].set_True()
                self.gliderState["c_gps_on"] = 0  # switch off gps
        else:  # end
    ...
    def beBusy(self, fsm):
        ...
        if (self.gliderState["m_present_time"] - self.__busytime) > self.gliderState[
            "_init_dive_time"
        ]:
            self.b_arg["resume"].set_True()
...
class Dive_to(DiveClimbBehavior):
    ...
    def init(self):
        ...
        # condition when to stop
        self.b_arg["stop_when"].addCondition(
            "m_depth", ">", self.target_depth, (0, None)
        )
        CLRS.r(self.behaviorName + ": " + self.fsm.current_state)
...
class Set_heading(WhenBehavior):
    ...
    def Active(self, fsm):
        ...
        self.gliderState["m_lmc_x"] = 0.0
        self.gliderState["m_lmc_y"] = 0.0
        ...
        if self.use_heading == 1:
            ...
            raise ValueError(
                'Unimplemented value for "use_heading" in "Set_heading" behavior'
            )
...
class Yo(WhenBehavior):
    def __init__(
        ...
        WhenBehavior.__init__(
            self, self.start_when, when_secs=None, when_wpt_dist=None
        )  # <---
    ...
    def init(self):
        ...
        self.fsm.add_transition("stop_when", "Active", self.Complete, "Complete")
        self.fsm.add_transition(
            "start_when", "Active", action=self.SubState, next_state="Active"
        )
        self.fsm.add_transition("resume", "Complete", self.resume, "UnInited")
        # if nothing applies, stay where you are
        self.fsm.add_transition_any("UnInited", action=None, next_state=None)
    ...
    def Initialise(self):
        CLRS.r(
            self.behaviorName + ": " + self.fsm.current_state
        )  # +"->"+fsm.next_state)
        self.current_profile = -1
        # reset some values for m_speed calcualtion
        divestate = Dive_to(
            target_depth=self.d_target_depth,
        ...
        divestate.init()
        climbstate = Climb_to(
            target_depth=self.c_target_depth,
...
class SGYo(Yo):
    ...
    def init(self):
        ...
        self.d_bpump_value, self.c_bpump_value = self.compute_buoyancy_values(
            self.d_target_depth,
    ...
    def compute_buoyancy_values(
        self, d_target_depth, target_divetime, bpump_offset, c_pitch_value
    ):
        S = 0.12
        Cd = self.drag_coefficient
        g = 9.81
        DV = 0.5 * Cd * S
        DV *= (2.0 * d_target_depth / target_divetime / sin(c_pitch_value)) ** 2
...
class Goto_list(WhenBehavior):
    ...
    def Steering(self, fsm):
        ...
        if self.list_stop_when == 7 and dist <= self.list_when_wpt_dist:
            ...
            if self.num_legs_to_run == self.achievedWaypoints:
                ...
                self.num_legs_to_run == -2
                and self.achievedWaypoints == self.num_waypoints
            ):
                self.b_arg["stop_when"].set_True()
            elif (
                self.num_legs_to_run == -1
                or self.num_legs_to_run > 0
                or self.num_legs_to_run == -2
            ):
                CLRS.r(
                    self.behaviorName + ": " + fsm.current_state + "->" + fsm.next_state
                )
    ...
    def __getWaypointForPickup(self):
        ...
        if found == -1:
            ...
            logger.warning(
                "Preset waypoint is not found in the waypoint list, assuming new mission start."
            )
            logger.warning(
                f"Waypoint: {self.waypoints[0][0]:6.3f}, {self.waypoints[0][1]:6.3f}"
            )
            found = 0
        return found
    ...
    def setWaypoint(self, r):
        ...
        CLRS.b(
            "New waypoint: %f %f. Mission time (hr): %.2f"
            % (
                self.gliderState["c_wpt_lat"],
                self.gliderState["c_wpt_lon"],
        ...
        self.activated_waypoints.append(
            (self.gliderState["m_present_time"], self.waypoints[r])
        )
...
class Prepare_to_dive(WhenBehavior):
    ...
    def Complete(self, fsm):
        ...
        self.gliderState["x_lmc_y_wpt_calc"] = self.gliderState["m_lmc_y"]
        self.gliderState["x_time_dive"] = self.gliderState["m_present_time"]
        self.gliderState["time_since_cycle_end"] = (
            0  # this is a poor choice, but I don't know where to do this better.
        )
...
class Sample(WhenBehavior):
    def __init__(self, start_when=0):
        self.start_when = start_when
