...
glider_model.initialise_gliderflightmodel(
    Cd0=0.20, mg=73.3, Vg=71.542e-3, T1=2052, T2=-35.5, T3=0.36
)
...
environment_model = glidersim.environments.VelocityRealityModel(
    "comet",
    download_time=24,
...
conf = glidersim.configuration.Config(
    "rapid-mooring.mi",  # the mission name to run
    ...
    sensor_settings=dict(
        c_wpt_lat=Nmea_lat,  # 5418.000,
        c_wpt_lon=Nmea_lon,  # 725.800,
        m_water_vx=0.0,
        m_water_vy=0.0,
    ),
    special_settings={
        "glider.gps.acquiretime": 100.0,  # how long the GPS should take to get a reading
        "mission_initialisation_time": 400,
        "initial_heading": 225,
    },  # how much time the glider needs to initialise.
    mission_start="pickup",
)  # if not set, a new mission is assumed, otherwise it is a continuation of
# a previous dive.

# Create a GliderMission object, specifying the glider hardware and environment.
GM = glidersim.glidersim.GliderMission(
    conf, verbose=True, glider_model=glider_model, environment_model=environment_model
)
